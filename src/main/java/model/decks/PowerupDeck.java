...
public class PowerupDeck {
    ...
    public PowerupDeck() {
        for (int i = 0; i == 0 || i == 12; i = i + 12) {
            ...
            this.powerups.get(i).addMove(new DamageEffect(POWERUP, new Player(), 0, false));

            this.powerups.add(new Powerup(TARGETING_SCOPE, new Ammo(Color.BLUE)));
            this.powerups.get(1 + i).addMove(new Pay());
            this.powerups.get(1 + i).addMove(new DamageEffect(POWERUP, new Player(), 0, false));

            this.powerups.add(new Powerup(TARGETING_SCOPE, new Ammo(Color.RED)));
            this.powerups.get(2 + i).addMove(new Pay());
            this.powerups.get(2 + i).addMove(new DamageEffect(POWERUP, new Player(), 0, false));
            ...
            this.powerups.get(6 + i).addMove(new MarkEffect(POWERUP, new Player(), 1, false));

            this.powerups.add(new Powerup(TAGBACK_GRENADE, new Ammo(Color.BLUE)));
            this.powerups.get(7 + i).addMove(new MarkEffect(POWERUP, new Player(), 1, false));

            this.powerups.add(new Powerup(TAGBACK_GRENADE, new Ammo(Color.RED)));
            this.powerups.get(8 + i).addMove(new MarkEffect(POWERUP, new Player(), 1, false));
